...
> [!review]
> 1.) What does it mean for a path joining two fixed points $\alpha$ and $\beta$ in a region $\Omega$ to be continuously deformable into another such path in $\Omega$ ? What is the analogous notion for two closed paths in $\Omega$, and for a closed path in $\Omega$ to be continuously deformable into a point?
> 2.) Why is the ambient region $\Omega$ essential to the notion of continuous deformability of paths in particular, how can the same pair of paths be mutually deformable in one region but fail to be mutually deformable in a subregion?
...
> [!exercise] Exercise 1: Continuous deformation of paths
> 
> For each of the following, justify the claim that the indicated continuous deformation can be carried out relative to the specified region:
> 1.) In $\Omega=\mathbb{C}$, the upper unit semicircle $\gamma_0(t)=e^{i t}, 0 \leq t \leq \pi$, is continuously deformable into the directed line segment from 1 to -1 .
> 2.) In the open unit disk $B_1(0)$, the polygonal path $\left[z_1, z_2, z_3\right]$ is continuously deformable into the directed line segment $\left[z_1, z_3\right]$, for the points $z_1, z_2, z_3$ shown in Figure 5.
> 3.) In the open unit disk $B_1(0)$, the arc $\gamma_0$ shown in Figure 6 is continuously deformable into the triangular path $\left[z_1, z_2, z_3\right]$.
> 4.) In the annular region $\Omega$ shown in Figure 7 , the circle $\gamma_0$ is continuously deformable into the circle $\gamma_1$ relative to $\Omega$.
> 5.) In the region $\Omega$ shown in Figure 8, the ellipse $\gamma_0$ is continuously deformable into a point relative to $\Omega$.
> 6.) Let $\Omega$ be a disk with three points $z_1, z_2, z_3$ removed. The circle $\gamma_0$ shown in Figure 9 is continuously deformable relative to $\Omega$ into the curve $\gamma_1$ consisting of three small circles centered at $z_1, z_2, z_3$ respectively, connected by line segments traversed in both directions.
...
# 3.3.2 Mathematical Definition of Deformation of Paths



> [!review]
> 1.) How is the intuitive notion of continuously deforming one path into another in a region $\Omega$ made mathematically precise? Give the definition in each of the cases where the paths join two fixed points $\alpha$ and $\beta$, where the paths are closed, and where one of the paths is a single point of $\Omega$.
> 2.) Prove that the relation of continuous deformability among paths in a region $\Omega$ is symmetric and transitive. That is, (i) if $\gamma_0$ is continuously deformable into $\gamma_1$ relative to $\Omega$, then $\gamma_1$ is continuously deformable into $\gamma_0$ relative to $\Omega$; and (ii) if $\gamma_0$ is continuously deformable into $\gamma_1$ and $\gamma_1$ into $\gamma_2$ (both relative to $\Omega$ ), then $\gamma_0$ is continuously deformable into $\gamma_2$ relative to $\Omega$.
...
> [!exercise] Exercise 2: Deformation of paths by continuous mappings
> 
...
> [!review]
> 1.) What does it mean for a subset $S$ of the complex plane to be convex?
> 2.) For a convex region $\Omega$ in $\mathbb{C}$, what continuous deformations among paths with common endpoints, among closed paths, and between a closed path and a single point are always possible? Give an explicit formula for the deformation mapping, and prove your answer.
...
> [!theorem] Theorem 1: Deformation of Paths in Convex Sets
...
> [!exercise] Exercise 3: Deformation of paths in convex sets
> 
> Show that in the open disk of radius 4 centered at the origin, the closed paths $\gamma_0(t)=e^{2 \pi i t}+ e^{4 \pi i t}$ and $\gamma_1(t)=3 e^{2 \pi i t}, 0 \leq t \leq 1$, are continuously deformable into one another by constructing an explicit continuous mapping of the unit square that realizes the deformation.
...
# 3.3.3 Cauchy's Theorem




> [!review]
> 1.) Let $f$ be a function analytic on a region $\Omega$. How does continuous deformability of two paths in $\Omega$ relate the values of the path integrals of $f$ along them? In particular, what does this imply when a closed path in $\Omega$ is continuously deformable to a point of $\Omega$ ?
> 2.) If $f$ is analytic on an open disk in $\mathbb{C}$, what value does $\int_\gamma f(z) d z$ take for a closed path $\gamma$ in that disk? What specialization does this give for entire functions integrated over an arbitrary closed path in C? Prove your answers.
...
> [!theorem] Theorem 2: Cauchy's Integral Theorem for Deformable Paths
...
> [!corollary] Corollary 1: Path Integrals of Functions Analytic on Disks
...
# 3.3.4 Simple Paths and Simply Connected Regions



> [!review]
> 1. Rather than checking case by case whether two closed paths in a region are mutually continuously deformable, we would like to identify regions where any two closed paths are automatically mutually continuously deformable. What are three equivalent conditions on a region $\Omega \subset \mathbb{C}$ that characterize when this is the case? Prove the equivalence.
> 2. What is a simply connected region, and what is a multiply connected region? What is the geometric intuition behind simply connected regions, and how does this intuition agree with the fact that every convex set is simply connected?
> 3. If $f$ is analytic on a simply connected region $\Omega$, how does $\int f(z) d z$ behave along paths in $\Omega$ ? In particular, what can be said about $\int_{\gamma_0} f(z) d z$ versus $\int_{\gamma_1} f(z) d z$ for two paths joining the same endpoints, and about $\int_\gamma f(z) d z$ for a closed path $\gamma$ in $\Omega$ ? Prove your answer.
> 4. If $f$ is analytic on a simply connected region $\Omega$, must $f$ have an antiderivative in $\Omega$ ? If so, how can one be constructed? Prove your answer.
> 5. What does it mean for a closed path to be simple?
> 6. What does the Jordan curve theorem say about a simple closed path in $\mathbb{C}$ ?
> 7. Given a simple closed path in $\mathbb{C}$, how are its positive and negative orientations defined, and how does this generalize counterclockwise versus clockwise orientation for circles?
> 8. Let $C$ be a simple closed path in $\mathbb{C}$. What can be said about the interior of $C$ as a region, and about the deformability of $C$ within any region containing $C$ and its interior?
> 9. If $f$ is analytic on a simple closed path $C$ and its interior, what is $\int_C f(z) d z$ ? What about $\int_\gamma f(z) d z$ for any closed path $\gamma$ (not necessarily simple) contained in the interior of $C$ ? Prove your answer.
...
> [!definition] Definition 1: Simply Connected Region, Multiply Connected Region
...
> [!theorem] Theorem 3: Cauchy's Theorem for Simply Connected Regions
...
> [!theorem] Theorem 4: Existence of Antiderivatives
...
> [!definition] Definition 2: Simple Path
...
> [!theorem] Theorem 5: Cauchy's Theorem for Simple Paths
...
# 3.3.5 Multiply Connected Regions



> [!review]
> Consider a simple closed positively-oriented path $C$ in $\mathbb{C}$, together with simple closed positively-oriented paths $C_1, C_2, \ldots, C_n$ contained in the interior of $C$ such that the interiors of the $C_j$ 's are pairwise disjoint. Suppose $f$ is analytic on a region $\Omega$ containing $C$ , all the $C_j$, and the region interior to $C$ and exterior to every $C_j$. How does $\int_C f(z) d z$ relate to the integrals $\int_{C_j} f(z) d z$ ? Prove your answer.
...
> [!theorem] Theorem 6: Cauchy's Theorem for Multiply Connected Regions
...
> [!exercise] Exercise: 4 A useful integral
> 
...
> [!exercise] Exercise 5: Path integrals of rational functions
> 
> Compute
> 
> $$
...
> [!exercise] Exercise 6: A path that is not simple
> 
...
> In problems 12-31, evaluate the given integral. Indicate clearly the version of Cauchy's theorem or the result from Section 3.3 that you are using.
...
> [!exercise] Exercise 16: Homotopy is an equivalence relation
> 
> 36.  (a) Given a path $\gamma_{0}(t)$ in a region $\Omega$, show that $H(t, s)=\gamma_{0}(t)$ will continuously deform $\gamma_{0}$ to itself, relative to $\Omega$. Thus all curves are homotopic to themselves. (In other words, homotopy is a reflexive relation.)
...
> [!exercise] Exercise 17: Proof of Proposition 1
> 37.  Here we prove that properties (i), (ii), and (iii) in Proposition 1 are equivalent; that is, if one of them holds for a region $\Omega$, then all of them must hold. We show that (i) ⇒ (iii) ⇒ (ii) ⇒ (i).
