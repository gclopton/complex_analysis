...
> [!review]
> 1. What is a star region in $\mathbb{C}$ ?
> 2. If $f$ is analytic on a star region $\Omega$, what can be said about $\int_{\Gamma} f(z) d z$ for a closed path $\Gamma$ in $\Omega$ ? Prove your answer.
> 3. If $f$ is a continuous complex-valued function on a closed and bounded set $S \subset \mathbb{C}$, what can be said about $f[S]$ ? What can be said about $|f|$ on $S$ ?
> 4. Define the distance between two nonempty sets $K, S \subset \mathbb{C}$. If $K$ is closed and bounded, $S$ is closed, and $K$ and $S$ are disjoint, what can be said about the distance between $K$ and $S$ ?
> 5. What does it mean for a function $f$ on a set $S \subset \mathbb{C}$ to be uniformly continuous, and how does this differ from the condition that $f$ be continuous at every point of $S$ ? Under what conditions on $S$ does continuity of $f$ on $S$ imply uniform continuity on $S$ ?
> 6. Let $\Omega \subset \mathbb{C}$ be a region and let $f$ be analytic on $\Omega$. If $\gamma_0$ and $\gamma_1$ are two paths in $\Omega$ that are mutually continuously deformable relative to $\Omega$ - either joining the same two endpoints, or both closed - what can be said about $\int_{\gamma_0} f(z) d z$ and $\int_{\gamma_1} f(z) d z$ ? Prove your answer.
...
> [!theorem] Cauchy's Integral Theorem for Star Regions
> Suppose that $f$ is analytic on a star region $\Omega$. Then
> 
> $$\int_{\Gamma} f(z) d z=0$$
> 
> for all closed paths $\Gamma$ in $\Omega$.

**Proof** According to Theorem 2, Section 3.3, it is enough to construct an antiderivative of $f$ in $\Omega$. Let $z_{0}$ be a point in $\Omega$ that connects to all other points in $\Omega$ via line segments (see _Figure 2_). 
...
For $z$ in $\Omega$, define $F(z)=\int_{\left[z_{0}, z\right]} f(\zeta) d \zeta$. Parametrizing the line segment by $\gamma(t)=(1-t) z_{0}+t z, 0 \leq t \leq 1$, we have $\gamma^{\prime}(t)=\left(z-z_{0}\right) d t$, and thus
...
For our next result, we define the notion of distance between two sets. If $K$ and $S$ are two nonempty sets, the distance between $K$ and $S$ is defined to be the smallest value of $|z-w|$ where $z$ is in $K$ and $w$ is in $S$ (_Figure 3_). 
...
If a smallest value is not attained, we take the distance to be the largest number $\delta$ such that $|z-w|$ is always greater than or equal to $\delta$.
...
Having chosen $n$, construct a grid over $Q$ by subdividing the sides into $n$ equal subintervals (_Figure 4_). 


> [!figure] Figure 4
> 
> ![700](./images/858d24a8-ffa3-45ac-9ebe-5fc1408bbf28-061_699_1483_302_574.jpg)
...
This is a simplicial network, where each smaller square in this network is called a simplicial square. 



Label the points on the grid by
...
which is contained in $B_{\delta}\left(z_{j k}\right)$ (see _Figure 5_). Since $f$ is analytic on $B_{\delta}\left(z_{j k}\right)$, Cauchy's theorem in a disk yields

$$
\int_{\sigma_{j k}} f(z) d z=0
$$

In _Figure 5_, we show a typical closed polygonal path obtained by joining the images of the four corners of one simplicial square. 
...
By construction, these polygonal paths are contained in disks of radius $\delta$ in ? In _Figure 6_, after summing all the path integrals and canceling those that are traversed ${ }^{\text {in }}$ opposite direction, the only ones that remain are those that correspond to the boundary of $Q$.
...
Adding the integrals over all the $\sigma_{j k}$ 's, we note the following. The integrals over the segments that correspond to internal sides of the simplicial squares cancel, since each internal segment is traversed twice in opposite directions. Thus the only noncanceling integrals are those over line segments that correspond to external sides of the simplicial squares, that is, the boundary of $Q$.
...
where $\gamma_{0, j}$ is the portion of the path $\gamma_{0}$ that joins the points $z_{j 0}$ and $z_{j+1,0}$ (_Figure 6_). Adding equations (9) as $j$ runs from 0 to $n-1$, we get
...
# 3.1 Appendix: Differentiation of Path Integrals



> [!review]
> 1. When is it legitimate to interchange the order of two iterated path integrals? That is, under what conditions on $f(z, \zeta)$ is $\int_{\gamma_1} \int_{\gamma_2} f(z, \zeta) d \zeta d z=\int_{\gamma_2} \int_{\gamma_1} f(z, \zeta) d z d \zeta$ ? Prove your answer.
> 2. Let $\gamma$ be a path in $\mathbb{C}$ and $\Omega \subset \mathbb{C}$ a region. Under what conditions on $f(z, \zeta)$ does the path integral $\phi(z)=\int_\gamma f(z, \zeta) d \zeta$ define a continuous function of $z$ on $\Omega$ ? Prove your answer.
> 3. Let $\gamma$ be a path in $\mathbb{C}$ and let $\Omega \subset \mathbb{C}$ be a region. Under what conditions on $f(z, \zeta)$ is the function $F(z)=\int_\gamma f(z, \zeta) d \zeta$ analytic on $\Omega$ ? In that case, what is $F^{\prime}(z)$ ? Prove your answer.
> 4. Under what conditions on $u$ does the integral $\phi(y)=\int_a^b u(x, y) d x$ define a continuous function of $y$ on $[c, d]$ ? Under what additional conditions is $\phi$ differentiable on $(c, d)$, and what is $\phi^{\prime}(y)$ ? Prove your answer.
...
> [!theorem] Theorem 2: Fubini's Theorem for Path Integrals
...
> [!theorem] Theorem 3: Continuity of Path Integrals
...
> [!theorem] Theorem 4: Differentiation of Path Integrals
...
> [!theorem] Theorem 5: Continuity and Differentiation of Integrals
...
> $$
> 
> Then $\phi(y)$ is continuous on $[c, d]$ (_Figure 8_). 
> 
> 
> > [!figure] Figure 8
> > 
...
> 
> 
> (ii) If $u$ is real-valued and continuous on $R$ and $\frac{\partial u}{\partial y}$ is also continuous on $R$, then $\phi(y)$ is differentiable on $(c, d)$ with
> 
> $$
...
> 
> For $z=x+i y$ in $B_{R}\left(z_{0}\right)$ (see _Figure 9_), define
> 
> $$
