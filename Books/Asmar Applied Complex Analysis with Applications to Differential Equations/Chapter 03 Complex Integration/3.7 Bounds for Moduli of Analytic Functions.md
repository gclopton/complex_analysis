...
> [!review]
> 1. If $f$ is analytic on an open disk $B_R\left(z_0\right)$ and $|f(z)| \leq M$ throughout the disk, how large can the derivatives $f^{(n)}\left(z_0\right)$ of $f$ at the center be? Give a precise upper bound for $\left|f^{(n)}\left(z_0\right)\right|$ in terms of $M, R$, and $n$, and prove your answer.
> 2. Can an entire function be bounded on $\mathbb{C}$ without being constant? Prove your answer.
> 3. If $f$ is entire and $|f(z)| \rightarrow \infty$ as $|z| \rightarrow \infty$, must $f$ have at least one zero in $\mathbb{C}$ ? Prove your answer.
> 4. Does every polynomial $p(z)$ of degree $n \geq 1$ with complex coefficients have a zero in $\mathbb{C}$ ? If so, how many zeros (counted according to multiplicity) must it have? Prove your answer.
...
> [!theorem] Theorem 1: Cauchy's Estimate
...
> [!theorem] Theorem 2: Liouville's Theorem
...
> [!theorem] Theorem 3: Fundamental Theorem of Algebra
...
# 3.7.1 Maximum and Minimum Principles


> [!review]
> 1. Is the value of an analytic function at a point equal to its average over any circle centered at that point, provided the closed disk bounded by the circle lies in the domain? Prove your answer.
> 2. Is the modulus of an analytic function at a point bounded above by its average modulus over any such circle? Prove your answer.
> 3. If a continuous real-valued function on a closed interval is bounded above by $M$ (alternatively, bounded below by $M$ ) and its average is $M$, must it be identically $M$ ? Prove your answer.
> 4. Can the modulus of a non-constant analytic function attain a maximum at an interior point of its domain? Prove your answer.
...
> [!theorem] Theorem 4: The Maximum Modulus Priciple
...
> [!exercise] Exercise 1: Failure of the maximum principle on unbounded regions
>Let $\Omega=\{z: \operatorname{Re} z>0$ and $\operatorname{Im} z>0\}$ be the open first quadrant, with boundary consisting of the nonnegative real and imaginary axes. Let $f(z)=e^{-i z^2}$. Show that $|f(z)|=1$ on the boundary of $\Omega$, but that $|f|$ is unbounded on $\Omega$. Explain why this does not contradict the maximum modulus principle.




Let $\Omega=\{z: \operatorname{Re} z>0$, and $\operatorname{Im} z>0\}$ be the first quadrant, bounded by the semi-infinite nonnegative $x$ and $y$-axes. Let $f(z)=e^{-i z^{2}}=e^{-i\left(x^{2}-y^{2}+2 i x y\right)}=$ $e^{-i\left(x^{2}-y^{2}\right)} e^{2 x y}$. We have
...
> [!review]
> 1. On a bounded region, must the modulus of an analytic function (continuous on the boundary) attain its maximum on the boundary, with strict inequality in the interior unless $f$ is constant? Prove your answer.
> 2. Can the modulus of a nonvanishing, non-constant analytic function attain a minimum in the interior of its domain? Prove your answer.
> 3. On a bounded region, must the modulus of a nonvanishing analytic function (continuous on the boundary) attain both its maximum and minimum on the boundary, with strict inequalities in the interior unless $f$ is constant? Prove your answer.
> 4. If an analytic function on the open unit disk maps into the closed unit disk and satisfies $f(0)=0$, what upper bounds hold for $|f(z)|$ and $\left|f^{\prime}(0)\right|$, and what must $f$ be if equality is achieved in either bound? Prove your answer.
...
> [!corollary] Corollary 2 (Maximum Modulus Principle)
...
> [!theorem] Theorem 5: The Minimum Modulus Principle
...
**Proof** Apply the maximum modulus principle to $g=\frac{1}{f}$. Combining the previous two results, we obtain the following principle.
...
> [!corollary] Corollary 3 (Maximum Minimum Modulus Principle)
...
> [!exercise] Exercise 2: Maximum and minimum values
> 
...
> [!exercise] Exercise 3 Linear fractional transformations
> 
