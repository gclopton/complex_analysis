...
459
(b) On the interval $-1 \leq x<1$, the graph consists of two straight lines (_Figure 3_). We have
$$
...
They are $2 \pi$-periodic, and orthogonal on the interval $[0,2 \pi]$. Recal orthogonality properties of the trigonometric system from problem 12 tion 3.2 (in what follows, the indices $m$ and $n$ are nonnegative integer
...
# problems 7.1
In problems 1-2, find a period of the given function and sketch its graph.
1. (a) $\cos x$,
(b) $\cos \pi x$,
(c) $\cos \frac{2}{3} x$,
(d) $\cos x+\cos 2 x$.
...
In problems 3-6, find a formula that describes the function in the accompan figure (_Figures 5-8_).
3.
...
(b) Conclude from (a) that $f$ is not periodic. Does this contradict problem 7? function $f$ is called almost periodic. These functions are of considerable inte and have many useful applications.
...
10. With the help of problem 9, determine the period of the given function.
(a) $\sin 2 x$
(b) $\cos \frac{1}{2} x+3 \sin 2 x$
(c) $\frac{1}{2+\sin x}$
(d) $e^{\mathrm{c}}$

In problems 11-14, a $\pi$-periodic function is described over an interval of lengt In each case plot the graph over three periods and compute the integral
$$
