...
> [!theorem] Theorem 1: Convergence of Power Series
...
Thus the series converges inside the circle of convergence and diverges outside the circle of convergence. On the circle of convergence, the series may converge at some or all or no points (_Figure 1_).
...
> [!exercise] Exercise: 1 Computing the radius of convergence
> 
...
> [!theorem] Theorem 2: Uniform Convergence of Subdisks
...
Since the series converges uniformly on $S_{r}\left(z_{0}\right)$ and $S \subset S_{r}\left(z_{0}\right)$ it follows that the series converges uniformly on $S$.
...
> [!theorem] Theorem 3: Term-by-Term Integration
...
> [!exercise] Exercise: 2 Term-by-term integration
> 
...
> [!theorem] Theorem 4: Analyticity and Termwise Differentiation of Power Series
...
> [!exercise] Exercise: 3 Term-by-term differentiation
> 
...
> [!exercise] Exercise: 4 Matching series
> 
...
Many important functions in applied mathematics are defined using power series. The exponential function is one example. In what follows we will introduce the Bessel functions using power series and derive some of their interesting properties. Bessel functions are **solution**s of the Bessel differential equation, which is an equation that arises in solving classical partial differential equations, including Laplace's equation on disks and cylindrical regions (see Chapter 9).


> [!exercise] Exercise: 5 Bessel functions of integer order
> 
...
**Solution** (a) Since $J_n(z)$ is defined by a power series, to prove that it is entire, it suffices by Theorem 4 to show that the power series converges for all $z$. Using the ratio test, we have for $z \neq 0$

$$
\begin{aligned}
\rho & =\lim _{k \rightarrow \infty}\left|\frac{a_{k+1}}{a_k}\right|=\lim _{k \rightarrow \infty}\left|\frac{z^{2(k+1)+n}}{2^{2(k+1)+n}(k+1)!(k+1+n)!} \frac{2^{2 k+n} k!(k+n)!}{z^{2 k+n}}\right| \\
...
Bessel functions of arbitrary order $p>0$ will be defined and studied in the exercises. 
...
We now record some immediate consequences of Theorem 4.



> [!corollary] Corollary 1: Taylor Coefficients
...
> [!theorem] Theorem 6: Cauchy-Hadamard Formula
...
> [!exercise] Exercise: 6 Cauchy-Hadamard formula
> 
> <span style="color:#ff0000; font-weight:bold">Formulate exercise statement</span>
...
> [!exercise] Exercise 7
> In problems 1-12, find the radius, disk, and circle of convergence of the given power series. Use the Cauchy-Hadamard formula in problems 9-12.
> 
...
> [!exercise] Exercise 11: Cauchy products of power series
> 21. Suppose that $\sum_{n=0}^{\infty} a_{n}\left(z-z_{0}\right)^{n}$ has radius of convergence $R_{1}>0$ and $\sum_{n=0}^{\infty} b_{n}\left(z-z_{0}\right)^{n}$ has radius of convergence $R_{2}>0$. Show that their Cauchy product is
> 
> $$
...
> [!exercise] Exercise 13: Project Problem
> 
> 23. The gamma function. For $\operatorname{Re} z>0$, define
> 
> $$
...
> [!exercise] Exercise 14: Project Problem
> 
> 25. The gamma function (continued). In this problem, you are asked to prove the formula
> 
> $$
...
> $$
> 
> All complex powers in this problem are principal branches.
