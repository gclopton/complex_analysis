...
> [!definition] Definition 1: Uniform Convergence
...
> [!exercise] Exercise: 1 Pointwise versus uniform convergence
> 
...
(b) _Figure 1_ suggests that the sequence does not converge to 0 uniformly on $[0,1]$. 
...
To confirm this, let us see how large $\left|f_{n}(x)\right|$ can get on the interval $[0,1]$. For this purpose, we compute the derivative
...
We have $M_{n}=f_{n}\left(\frac{1}{10}\right)$ and we know that $f_{n}\left(\frac{1}{10}\right) \rightarrow 0$ as $n \rightarrow \infty$ from part (a). Thus $M_{n} \rightarrow 0$, implying that $f_{n} \rightarrow 0$ uniformly on $[0.1,1]$.
...
> [!theorem] Theorem 1: Continuity and Uniform Convergence
...
> [!exercise] Exercise: 2 Failure of uniform convergence
> 
...
**solution** The function $f_{n}(z)$ with $n=5$ is depicted in _Figure 3_. 



> [!figure] Figure 3
> 
> ![[Screenshot 2026-04-02 at 11.01.50 PM.png]]
> Figure 3 Graph of $f_n(z)$ (with $n=5$ ) in Example 2.




It is clear that each $f_{n}(z)$ is continuous on $|z| \leq 1$ and
...
> [!Theorem] Theorem 2: Sequential Limits and Integrals
...
> [!corollary] Corollary 1: Term-by-Term Integration of Series
...
> [!theorem] Theorem 3: Weierstrass M-Test
...
> [!exercise] Exercise: 3 Weierstrass $M$-test
> 
...
> [!exercise] Exercise 4: Uniform convergence and the geometric series
> 
...
# 4.2.1 Sequences and Series of Analytic Functions
...
> [!theorem] Theorem 4: Sequential Limits and Derivatives
...
> [!exercise] Exercise: 5 Failure of termwise differentiation
> 
...
> [!corollary] Corollary 2: Term-by-Term Differentiation of Series 
...
> [!exercise] Exercise 6: Term-by-term differentiation of the geometric series
> 
> <span style="color:#ff0000; font-weight:bold">Exercise statement needed</span>

The geometric series $\sum_{n=0}^{\infty} z^{n}$ converges uniformly to $\frac{1}{1-z}$ on any disk $|z| \leq r<1$, by Example 4. It will then converge uniformly on any closed disk contained in the open disk $|z|<1$. Applying Corollary 2, we may differentiate term by term in the open disk $|z|<1$ and get
...
> [!exercise] Exercise 7: Termwise differentiation and integration
> 
...
(b) Note from (a) that the series will be uniformly convergent on any closed and bounded subset of the punctured plane $\mathbb{C} \backslash\{0\}$, since any such set can be contained in an annular region $|z| \geq \rho$ (_Figure 4_). 
...
Applying Corollary 2, it follows that $u(z)$ is analytic on $\mathbb{C} \backslash\{0\}$.
...
$$
\int_{C_R(0)}\left\{\sum_{n=0}^{\infty} \frac{1}{n!z^n}\right\} d z=2 \pi i
$$
...
> [!exercise] Exercise 8
> In problems 1-8, (a) find the pointwise limit of the given sequence. (b) Determine if the sequence converges uniformly on the given interval. (c) If the sequence does not converge uniformly on the given interval, describe some subintervals on which uniform convergence takes place.
...
> In problems 13-22, use the Weierstrass $M$-test to show that the given series converges uniformly on the given region.
...
> 24. (a) Does $\sum_{n=0}^{\infty} z^{n}$ converge uniformly on $\left|z-\frac{1}{2}\right|<\frac{1}{6}$ ?
