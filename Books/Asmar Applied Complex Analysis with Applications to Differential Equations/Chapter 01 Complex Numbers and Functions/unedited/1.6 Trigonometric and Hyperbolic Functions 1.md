...
> 1. What real-variable identity motivates defining $\cos z$ and $\sin z$ by the formulas $\frac{e^{i z}+e^{-i z}}{2}$ and $\frac{e^{i z}-e^{-i z}}{2 i}$ ? Why does that identity make these definitions the natural choice?
> 2. Show that when $z=\theta$ is real, the definitions $\cos z=\frac{e^{i z}+e^{-i z}}{2}$ and $\sin z= \frac{e^{i z}-e^{-i z}}{2 i}$ reduce to the standard real cosine and sine functions.
> 3. The text notes that $\cos z$ and $\sin z$ are not bounded in absolute value. In what two respects does this mark a departure from their real counterparts, and which feature of the complex exponential is responsible?
